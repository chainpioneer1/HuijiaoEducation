...
<div class="base-container" style="top: 0;">
    <div style="width: 100%;height: 1035px;overflow: hidden; margin-bottom: 0px;">
        ...
        <?php
        $additional = $contentItem->additional_info;
        if ($additional == null || $additional == '') $additional = '';
        else $additional = base_url() . $additional;
        if ($additional != '') { ?>
            <a class="title-subcontent" href="<?= base_url().'resource/additionalPreviewPlayer/'.$contentItem->id ?>" target="_blank">附件内容</a>
        <?php } ?>
        ...
        <div class="bottom_buttons_section">
            <div class="buttons-container">
                ...
                <span class="like_span"><?= $like_num ?></span>
                ...
                <span class="favorite_span"><?= $favorite_num ?></span>
                ...
                <?php if ($contentItem->is_download != 1 || $this->session->userdata('user_type') != '1') { ?>
                    ...
                    <span class="download_span" style="color: #888;">下载</span>
                <?php } else { ?>
                    <a class="button-elem" id="download_btn" data-favorite="<?= $usage->is_favorite ?>" href="" download
                       target="_blank">
...
<input class="courseListInfo" style="display:none;" value='<?php echo json_encode($courseList);?>'>
<script>

    var courseList = JSON.parse($('.courseListInfo').val());
    var pretitle = "<?= $title?>";
    ...
    $('#favorite_btn').click(function () {
        ...
        jQuery.ajax({
            ...
            success: function (res) {
                ...
                $('.buttons-container .favorite_span').text(res.data.favorite_num);
                if (res.status == 'success') {
                    $('#favorite_btn').attr('data-favorite', favorite)
        ...
        jQuery.ajax({
            ...
            success: function (res) {
                if (res.status == 'success') {
                    ...
                    $('.buttons-container .like_span').text(res.data.like_num);
