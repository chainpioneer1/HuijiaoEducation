$('.bg').css({background: 'url(' + imageDir + 'bg.png) no-repeat'});
$(function () {
    var content_html = "";
    for (var i = 0; i < packageList.length; i++) {
        ...
        if (item.owner_type == 0 + '')
            content_html += '<div class="list_item" itemid="' + item.id + '" item_type="online"' +
                'style="background:url(' + baseURL + '/assets/images/classroom/kuaile' + item.id + '.png);"></div>';
       }
    for (var i = 0; i < packageList.length; i++) {
        var item = packageList[i];
        if (item.owner_type == loginUserId && item.lesson_status == '0')
