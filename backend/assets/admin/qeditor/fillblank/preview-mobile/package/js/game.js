...
(function (game) {
    ...
    game.initGame = function () {
        ...
        $('.section[data-type="submit"]').on(upEvent, function () {
            ...
            if (isSuccess) {
                $('.blank-item').css({color:'#00b91a','border':'2px solid #00b91a'});
...
function getQuestionStudentAnswerArr() {
    var isSuccess = true;
    var answerItems = $('input.blank-item');
    if(_answerArray.length==0) isSuccess = false;
    var studentAnswer = [];
    console.log('-- getQuestionStudentAnswerArr / _answerArray : ', _answerArray);
    console.log('-- getQuestionStudentAnswerArr / answerItems : ', answerItems);
    for (var i = 0; i < _answerArray.length; i++) {
        var isUserChecked = $(answerItems[i]).val();

        var realAnswer = _answerArray[i].content.split('|');
        var isRight = false;
        for(var kk=0; kk<realAnswer.length; kk++){
            if(isUserChecked == realAnswer[kk]) isRight = true;
        }
        if(!isRight) isSuccess = false;
    ...
    console.log('-- getQuestionStudentAnswerArr / studentAnswer : ', studentAnswer);
    console.log('-- getQuestionStudentAnswerArr / isSuccess : ', isSuccess);
    return {
