...
(function (game) {
    ...
    game.initGame = function () {
                ...
                switch (idx) {
                    ...
                    case 3: // add option btn
                        ...
                        if (sel.getRangeAt && sel.rangeCount) {
                            if (curActiveElement.attr('data-type') == 'question') {
                                optionCnt++;
        ...
        $('.item-edit[data-type="question"]').on('input', function () {
            game.addOptionItem(optionCnt, false);
        });
    ...
    game.addOptionItem = function (cnt, isUpdate) {
        ...
        if (cnt < 0) return;
        var blankCnt = $('.blank-item').length;
        if (isUpdate && blankCnt>0) cnt = blankCnt+1;
        else cnt = blankCnt;
        ...
        // for(var idx=0; idx<cnt;idx++){
        //     content_html+='<div class="answer-item">' +
        //         '<input type="radio" name="ans_radio" data-id="'+idx+'" '+(idx==0?'checked':'')+'>'+keys[idx]+'</div>';
        // }
        // $('.item-edit[data-type="answer"]').html(content_html);

        // ***************** fill blank ***************
        if (isUpdate && optionCnt>0)
            pasteHtmlAtCaret(getTemplateHtml(keys[optionCnt - 1], ST.blank_item));
        for (var idx = 0; idx < cnt; idx++) {
            var blankElem = $($('.blank-item')[idx]);
            blankElem.attr('data-id', keys[idx]);
            blankElem.html(keys[idx]);
        }
        ...
        for (var idx = cnt; idx < $('.item-edit-row').length; idx++) {
            $('.item-edit-row[data-id="' + idx + '"]').remove();
            $('.blank-item[data-id="' + keys[idx] + '"]').remove();
        }
...
function setQuestionContent(content) {
    ...
    var totalBlank = parentElem.find('.blank-item');
    for (var k = 0; k < totalBlank.length; k++) {
        var newNode = document.createElement('div');
        var originalNode = totalBlank[k];
        for (var i = 0, l = originalNode.attributes.length; i < l; ++i) {
            var nodeName = originalNode.attributes.item(i).nodeName;
            var nodeValue = originalNode.attributes.item(i).nodeValue;
            newNode.setAttribute(nodeName, nodeValue);
        }
        newNode.innerHTML = (k+1);
...
function getQuestionContent() {
    ...
    contentElem.innerHTML = parentElem.html();
    console.log(parentElem.find('.blank-item').length);
    var totalBlank = parentElem.find('.blank-item');
    for (var k = 0; k < totalBlank.length; k++) {
    ...
    var newHtml = parentElem.html() + '';
    parentElem.html(contentElem.innerHTML);
    return newHtml;
}

function setAnswerContentArr(content) {
    var content = JSON.parse(content);
    startGame1.addOptionItem(content.length, false);
    ...
    for (var i = 0; i < optionArr.length; i++) {
        ...
        // answerArr[i].checked = content[i].is_checked;
    }
    return true;
}
...
function getQuestionInfo() {
    return {
        ...
        type: 2, // 0-multi selection, 1-yesno,  2-fill blank
