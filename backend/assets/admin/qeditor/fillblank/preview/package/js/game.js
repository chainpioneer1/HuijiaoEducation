...
(function (game) {
    ...
    game.initGame = function () {
        ...
        $('.section[data-type="submit"]').on(upEvent, function () {
            var isSuccess = true;
            var answerItems = $('.blank-item');
            if (_answerArray.length == 0) isSuccess = false;
            console.log(_answerArray, answerItems);
            for (var i = 0; i < _answerArray.length; i++) {
                var userAnswer = $('.blank-item[data-id="'+(i+1)+'"]').val();
                var realAnswer = _answerArray[i].content.split('|');
                var isRight = false;
                for(var kk=0; kk<realAnswer.length; kk++){
                    if(userAnswer == realAnswer[kk]) isRight = true;
                }
                if(!isRight) isSuccess = false;
            }
            if (isSuccess) {
                $('.blank-item').css({color:'#00b91a','border-color':'#00b91a'});
                clearInterval(_tmr);
                alert('太棒了, 你答对了!');
            } else {
                alert('这道题没有答对, 下次加油哦!');
    ...
    game.addOptionItem = function (content, isUpdate) {
        return;
        ...
        for (var idx = 0; idx < cnt; idx++) {
            content_html += '<div class="answer-item" data-id="' + idx + '">' +
...
function setQuestionContent(content) {
    ...
    $('.section span').css({padding:0,margin:0});
    $('.section input').css({width: 30, 'text-align': 'center',margin:0});
    $('.section input').off('input');
    $('.section input').on('input', function (object) {
        var that = $(this);
        var len = that.val().length;
        $(this).css({width: len * 9 + 30})
    });
}

function setQuestionAnswerArr(content) {
    content = JSON.parse(content);
    _answerArray = content;
    return;

    startGame1.addOptionItem(content);

    var answerArr = $('.section[data-type="answer"] .item-content');
    for (var i = 0; i < content.length; i++) {
        answerArr[i].innerHTML = content[i].content;
    }
    $('.section[data-type="answer"] .answer-item>div').off(upEvent);
    $('.section[data-type="answer"] .answer-item>div').on(upEvent, function (object) {
        var that = $(this);
...
function setQuestionInfo() {
    ...
    setQuestionAnswerArr(JSON.stringify(qInfo.ans));
    return true;
}